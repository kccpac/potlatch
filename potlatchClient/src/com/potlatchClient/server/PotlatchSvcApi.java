...
public interface PotlatchSvcApi {
	...
	public static final String THUMBNAIL_PARAMETER = "thumb";
	
	public static final String OWNER_PARAMETER = "owner";
	
	public static final String GIFT_ID_PARAMETER = "giftId";	
	
	public static final String OWNER_ID_PARAMETER = "ownerId";//OWNER_PARAMETER + "Id";
	
	public static final String ID_PARAMETER = "id";
	
	public static final String GIFT_CAT_PARAMETER = "category";
	
	public static final String POTLATCH_DATA_PATH = POTLATCH_SVC_PATH +
													"/{" + OWNER_ID_PARAMETER + "}" +
													"/{" + GIFT_ID_PARAMETER + "}" +
													"/gift";

	public static final String POTLATCH_QUERYDATA_PATH = POTLATCH_SVC_PATH +
														"/{" + GIFT_ID_PARAMETER + "}" +
														"/gift" +
														"/{" + GIFT_CAT_PARAMETER + "}";
	
	public static final String GIFT_TITLE_PARAMETER = "title";
	
	@GET(POTLATCH_SVC_PATH + "/UserId")
	public long getUserId(@Query("Username") String username);
	
	@GET(POTLATCH_SVC_PATH)
	public Collection<Gift> getGiftList(@Query("eMotionType") emotionType filterType);

	@POST(POTLATCH_SVC_PATH)
	public Gift addGift(@Body Gift v);	
	
	@Multipart
	@POST(POTLATCH_DATA_PATH)
	public PotlatchStatus setGiftData(
			@Path(OWNER_ID_PARAMETER) String ownerId,
			@Path(GIFT_ID_PARAMETER) String giftId,
			@Part(THUMBNAIL_PARAMETER) TypedFile giftThumbnail,
			@Part(DATA_PARAMETER) TypedFile giftData);

	
	@Streaming
    @GET(POTLATCH_QUERYDATA_PATH)
    public Response getData(
    		//@Path(OWNER_ID_PARAMETER) String oid,
    		@Path(GIFT_ID_PARAMETER) String iid,
    		@Path(GIFT_CAT_PARAMETER) String category
    		);
	
	@GET(POTLATCH_SVC_PATH  + "/giftById")
	public Gift findGiftById(@Query(GIFT_ID_PARAMETER) String giftId);
	
	@GET(POTLATCH_SVC_PATH  + "/giftByTitle")
	public Gift findGiftByTitle(@Query(GIFT_TITLE_PARAMETER) String title);
	
	@GET(POTLATCH_SVC_PATH  + "/giftListByOwnerId")
	public Collection<Gift> getGiftListByOwnerId(@Query(OWNER_ID_PARAMETER) String id);

	@POST(value=PotlatchSvcApi.POTLATCH_SVC_PATH+"/emotion")
	public Void eMotionByGift(
			@Query(value="id") long id,
			@Query(value="eMotionType") emotionType type);
	
	@POST(value=PotlatchSvcApi.POTLATCH_SVC_PATH+"/unEmotion")
	public Void UneMotionByGift(
			@Query(value="id") long id,
			@Query(value="eMotionType") emotionType type);
	
	@GET(value=PotlatchSvcApi.POTLATCH_SVC_PATH+"/getUsers")
	public Collection<String> getUsersWhoEmotionByGift(
			@Query(value="id") long id,
			@Query(value="eMotionType") emotionType type);
	
	@GET(value=PotlatchSvcApi.POTLATCH_SVC_PATH+"/getEmotionCount")
	public Collection <touchCount> getEmotionCountList(
			@Query(value="eMotionType") emotionType type);
	
	@GET(value=PotlatchSvcApi.POTLATCH_SVC_PATH+"/getUser")
	public UserEmotion queryUserData(
			@Query(value="userId") long userId,
			@Query(value="giftId") long giftId);
	
	@POST(value=PotlatchSvcApi.POTLATCH_SVC_PATH+"/setUser")
	public boolean setUserEmotion(
			@Query(value="userEmotion") UserEmotion	user);
